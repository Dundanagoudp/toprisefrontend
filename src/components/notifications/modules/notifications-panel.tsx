...
export function NotificationsPanel({
    ...
    try {
      ...
      if (response.success) {
        ...
        setNotificationList((prev) => {
          ...
          const updatedUnreadCount = updated.filter(
            (n) => !n.markAsRead
          ).length;
    ...
    switch (filter) {
      case "read":
        return notificationList.filter(
          (notification) => notification.markAsRead === true
        );
      case "unread":
        return notificationList.filter(
  ...
  const headerBadgeCount =
    unreadCount > 0 && unreadCount < allTotalCount ? unreadCount : 0;
                    ...
                    (filterType) => {
                      ...
                      if (isActive) {
                        if (filterType === "unread") {
                          ...
                          colorClasses =
                            "bg-[#d7e5fd] text-blue-800 hover:bg-[#c6dbfc]";
                        } else {
                          colorClasses =
