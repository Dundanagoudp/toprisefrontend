...
function CreatePopularVehicleModal({ open, onClose, onSuccess }: CreatePopularVehicleModalProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await createPopularVehicles(submitData);

      if (response && response.success) {
        showToast("Popular vehicle created successfully", "success");
        onSuccess?.();
        onClose();
      } else {
        throw new Error(response?.message || "Failed to create popular vehicle");
      }
    } catch (error: any) {
      console.error("Error creating popular vehicle:", error);
      const errorMessage = error?.response?.data?.message || error?.message || "Failed to create popular vehicle";
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-[500px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Create Popular Vehicle</DialogTitle>
        </DialogHeader>
...
export default CreatePopularVehicleModal;
