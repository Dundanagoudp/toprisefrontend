...
export default function PendingProductByDealer({
  searchQuery,
}: {
  searchQuery: string;
}) {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      try {
          const response = await getDealerProductsByPage(
            currentPage,
            itemsPerPage,
            "Pending"
          );
          if (response.data) {
            // Debug log to inspect product fields
            console.log("API Products Response:", response.data.products);
  ...
  const filteredProducts = React.useMemo(() => {
    ...
    // Only show products with live_status 'Pending'
    let filtered = paginatedProducts.filter((product: any) => product.live_status === "Pending");
